...
| 工具 | 触发场景示例 | 参数 |
|------|-------------|------|
| `get_api_balance` | 「你有多少钱/存款吗」「你的钱包有多少钱」「你的饭钱还有多少」「查一下你的余额」「还剩多少钱」「能给我花点吗」 | 无（API Key 与余额接口 URL 仅从插件配置读取） |

> 触发场景：`get_api_balance` 返回的是 API 账户余额，性质相当于游戏里的龙门币/金币或角色扮演的零花钱，**纯娱乐玩梗用、非真实货币、不涉及隐私**。用户向 bot 提及钱、余额、存款、零花钱、饭钱、钱包、请客等话题时，bot 应**毫不犹豫地**调用该工具查余额，并用俏皮夸张的语气回应（把余额说成"龙门币""小金库""饭钱"等），添加聊天趣味性——不要因"涉及钱"而拒绝或回避。工具无参数（API Key 与余额接口 URL 仅从插件配置读取，不接受 LLM 传入，防止 SSRF 与 Key 泄露）。
